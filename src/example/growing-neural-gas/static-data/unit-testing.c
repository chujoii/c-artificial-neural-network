...
int main ()
{
	...
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng); // add extract_groups_from_conn_ages before print_neuron for update group list (only visual changes)
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
	...
	update_neuron_conn_age (0, 1, 1, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}

	set_neuron_conn_age (0, 1, GNG_NOT_CONNECTED, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	printf ("\nsimple 6 neurons (all disconnected):\n");
	add_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);add_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);add_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);add_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	update_neuron_weight_vector(3, GNG_EPS_WINNER, example_sensor, GNG_DIMENSION_OF_SENSOR, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	calculate_distance_weight_sensor (example_sensor, GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng, distances);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
	...
	initialization (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing2_gng);
	add_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing2_gng);add_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing2_gng);add_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing2_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		for (int j=0; j<GNG_DIMENSION_OF_SENSOR; j++) {
	...
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing2_gng);
	...
	calculate_distance_in_mixed_space_weight_sensor (mixed_space_distances, example_sensor_with_angle, GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing2_gng, distances);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
	...
	update_neighbours_weights (2, GNG_EPS_NEIGHBOUR, example_sensor, GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	update_neighbours_weights (3, GNG_EPS_NEIGHBOUR, example_sensor, GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	inc_neighbours_conn_age (0, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	inc_neighbours_conn_age (3, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	set_neuron_conn_age (1, 2, GNG_INITIAL_CONNECTION_AGE, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	remove_old_conn_age (GNG_LIMIT_CONN_AGE, GNG_LIMIT_NETWORK_SIZE, testing2_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing2_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
	...
	find_and_del_neuron_with_min_utility_factor (GNG_K_UTILITY, GNG_LIMIT_NETWORK_SIZE, testing2_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing2_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing2_gng[i]);
	}
	...
	int index_max_local_error = find_neuron_index_with_max_local_error (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	...
	printf ("neighbour (for neuron number %d) index of neuron with max local error: %d\n",
		index_max_local_error,
		find_neighbours_index_with_max_local_error (index_max_local_error, GNG_LIMIT_NETWORK_SIZE, testing_gng));


	printf ("\nadaptive step: create new neuron:\n");
	adaptive_step_create_new_neuron (GNG_EPS_LOCAL_ERROR, GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	decrease_all_neuron_local_errors_and_utility_factor (GNG_FACTOR_BETA_DECREASE_LOCAL_ERROR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	extract_groups_from_conn_ages (GNG_LIMIT_NETWORK_SIZE, testing_gng);
	for (int i=0; i<GNG_LIMIT_NETWORK_SIZE; i++) {
		print_gng_neuron (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng[i]);
	}
	...
	result = write_gng_to_file ("knowledge-base.gng", GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	printf (" %s\n", (result == 0) ? "OK" : "Error");
	printf ("initialize (clean) gng\n");
	initialization (GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	printf ("read GNG from file knowledge-base.gng ...");
	result = read_gng_from_file ("knowledge-base.gng", GNG_DIMENSION_OF_SENSOR, GNG_LIMIT_NETWORK_SIZE, testing_gng);
	printf (" %s\n", (result == 0) ? "OK" : "Error");
